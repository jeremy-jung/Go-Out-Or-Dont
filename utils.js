...
const rateForState = (covidResponse, weatherResponse) => {
    ...
    let positiveIncrease = covidResponse.positiveIncrease;
    let negativeIncrease = covidResponse.negativeIncrease;
    let covidOutOfFive = rateCovidState(positiveIncrease, negativeIncrease);

    let uvIndex = weatherResponse.uv;
    let uvOutOfFive = rateUv(uvIndex);

    let temp_f = weatherResponse.temp_f;
    let tempOutOfFive = rateTemperature(temp_f);

    ratings["uv"] = Number(uvOutOfFive);
    ratings["temp"] = Number(tempOutOfFive);
    ratings["covid"] = Number(covidOutOfFive);

    // console.log(ratings["uv"] + ratings["temp"] + ratings["covid"]);
    if (ratings["uv"] + ratings["temp"] + ratings["covid"] < 10) {
        ratings["goout"] = false;
    }
    else {
        ratings["goout"] = true;
...
const rateForCountry = (covidResponse, weatherResponse) => {
    let ratings = {};

    let todayCases = covidResponse.todayCases;
    let covidOutOfFive = rateCovidCountry(todayCases);
...
module.exports.rateCovidCountry = rateCovidCountry;
module.exports.rateCovidState = rateCovidState;
module.exports.rateUv = rateUv;